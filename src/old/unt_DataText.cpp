...
//---------------------------------------------------------------------------
cls_DataText::cls_DataText(class cls_Log* l,const String& hs,const String& dl) : Log(l)
{ // Class constructor
...
// . . . Defaults
        ...
        // . . . Header chars
        HeaderChars = "«»()[]#"; // Ensure an initial good default
        HeaderChars = hs; // Try to assign the external String

        // . . . Data delimiters
        DataDelimiters = " \t;"; // Ensure an initial good default
        DataDelimiters = dl; // Try to assign the external String

        ShowMessage(DataDelimiters); //###

// . . . Other initializations
}

//---------------------------------------------------------------------------
//cls_DataText::~cls_DataText()
//{ // Class destructor
//        //
//}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
String __fastcall cls_DataText::ImportFrom(TStrings* Lines,const mat::stringlist& GivenStrDirs)
            ...
            if ( !IsDataReached ) // Line starts with a not-a-number char
               {
                ...
                if ( Line[1] == Prefix_Directive)
                   ...
                   {// Not a directive. Is it an header?
                    String Res = mat::GetDelimitedSubStrings(Tokens,Line,Brackets_Column);
                    if ( Res.IsEmpty() )
                       {// An header found
                        if ( IsHeaderFound )
                           {
...
//---------------------------------------------------------------------------
void __fastcall cls_DataText::ExportTo(TStrings* Lines)
...
// . . . Header
        ...
        for (U32 i=0; i<Columns.size(); i++) // For each column
            {
             Line += "  " + String(Brackets_Column[1]) + Columns[i].Label + " ";
             if ( Columns[i].XIndex!=int(i) )
                {
                 if ( Columns[i].XIndex>=0 ) Line += String(Brackets_Abscissa[1])+Columns[i].XLabel+String(Brackets_Abscissa[2])+" ";
                 else Line += String(Brackets_Abscissa[1])+(Columns[i].IdxScale==1?String(""):String(Columns[i].IdxScale))+Columns[i].IdxUnit+String(Brackets_Abscissa[2])+" ";
                }
             Line += String(Brackets_Unit[1]);
             if ( Columns[i].Scale != 1 ) Line += String(Columns[i].Scale);
             Line += Columns[i].Unit + String(Brackets_Unit[2]) + String(Brackets_Column[2]);
            }
        Lines->Add(Line);
        Lines->Add("");
...
//---------------------------------------------------------------------------
void __fastcall cls_DataText::LogColumnsSummary()
...
//---------------------------------------------------------------------------
void __fastcall cls_DataText::Split(String strpat, bool samewidth)
{ // Splitting columns
...
//---------------------------------------------------------------------------
void __fastcall cls_DataText::Reorder(String strseq)
...
//---------------------------------------------------------------------------
void __fastcall cls_DataText::UnMap()
...
//---------------------------------------------------------------------------
void __fastcall cls_DataText::ExecuteDirectives()
        ...
        for (U32 i=0; i<Directives.size(); i++)
              {
               ...
               else if ( dirstr[1] != Prefix_Directive )
                  {// What?? Something wrong!
...
//---------------------------------------------------------------------------
void __fastcall cls_DataText::set_DataDelimiters (const String& s)
{ // Set data separation delimiters String
        if ( s.IsEmpty() )
           {
            //mat::DisplayWarn("cls_DataText","I won\'t set DataDelimiters with an empty string!\nI\'ll leave it to: " + DataDelimiters);
            Log->SubEntry("cls_DataText:: I\'ll try to detect the data separators");
           }
        i_DataDelimiters = s;
}

//---------------------------------------------------------------------------
void __fastcall cls_DataText::set_HeaderChars (const String s)
{ // Set header chars
        if ( s.Length() != 7 )
             {
              mat::MsgWarn("cls_DataText","I won\'t set HeaderChars with the illegal String "+s+"\nI\'ll leave it to: " + HeaderChars);
             }
        else {
              Brackets_Column = s.SubString(1,2);
              Brackets_Abscissa = s.SubString(3,2);
              Brackets_Unit = s.SubString(5,2);
              Prefix_Directive = s[7];
             }
}
...
//---------------------------------------------------------------------------
void __fastcall cls_DataText::BuildDefaultColumns( int num )
...
//---------------------------------------------------------------------------
void __fastcall cls_DataText::DecodeHeader(const mat::stringlist& Tokens)
        ...
        for (U32 i=0; i<colcount; i++)
            {
             ...
             // Contains a unit?
             if ( mat::GetDelimitedSubStrings(Bracketed,Token,Brackets_Unit).IsEmpty() )
             ...
             if ( mat::GetDelimitedSubStrings(Bracketed,Token,Brackets_Abscissa).IsEmpty() )
